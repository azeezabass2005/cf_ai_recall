...
/**
 * HTTP client for the Recall Worker. Covers chat (LLM path) and direct
 * REST CRUD for reminders (manual form path, no LLM involvement).
 */
class RecallApi(private val context: Context) {
    ...
    private val client = HttpClient(OkHttp) {
        ...
        // user isn't greeted with a socket error instead of Recall's reply.
        install(HttpTimeout) {
    ...
    private val baseUrl: String get() = BuildConfig.RECALL_BASE_URL
    ...
    suspend fun chat(message: String, inputMode: String = "text"): ChatResponse {
        ...
        val response = client.post("$baseUrl/chat") {
            headers.append("X-Recall-Device", deviceId)
            setBody(
                ChatRequest(
                    message = message,
